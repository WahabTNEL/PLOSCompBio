...
%        John Cunningham  jcunnin@stanford.edu
  ...
  for j = 1:length(Tu)   
    ...
    [invM, logdet_M] = invPerSymm(gpuArray(K_big_inv + blkdiag(blah{:})), xDim,... 
    'offDiagSparse', true);
    invM=gather(invM);
    logdet_M=gather(logdet_M);
    ...
    blkProd = gpuArray(K_big(1:(xDim*Thalf), :)) *... 
                fillPerSymm(gpuArray(speye(xDim*Thalf, xDim*T) - blkProd), xDim, T);   
    blkProd=gather(blkProd);
