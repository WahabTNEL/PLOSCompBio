...
plotEachDimVsTime(seqTrain, 'xsm', result.binWidth);
% NOTES:
...
parfor i=1:6
    xDim=xrange(i);
  %neuralTraj(runIdx, data, 'method',  'pca', 'xDim', xDim, 'numFolds', numFolds);
  %neuralTraj(runIdx, data, 'method', 'ppca', 'xDim', xDim, 'numFolds', numFolds);
  neuralTraj(runIdx, data_d, 'method',   'fa', 'xDim', xDim, 'numFolds', numFolds, 'emMaxIters', 1000, 'binWidth', 250, 'kernSDList', kernSDList);
  neuralTraj(runIdx, data_d, 'method', 'gpfa', 'xDim', xDim, 'numFolds', numFolds, 'emMaxIters', 1000, 'binWidth', 250, 'kernSDList', kernSDList);
end
fprintf('\n');
%%
% NOTES:
...
kernSD = 501; % select kernSD for two-stage methods
...
% Results files are loaded from mat_results/runXXX/, where XXX is runIdx.
xDim = 10; % select state dimensionality
