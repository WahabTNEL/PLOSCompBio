function result = neuralTraj_NYU_v2(runIdx, dat, indD, indR, indEL, indTV,  varargin)
%
...
seq=dat;
  ...
  for cvf = 0:numFolds
    ...
    if cvf > 0
          ...
          seq_tv_train1=seq_tv(find(indTV<cvf-d | indTV>cvf+d));
          
          minlength=min([length(seq_d_train1) length(seq_r_train1) length(seq_el_train1) length(seq_tv_train1)]);
          ...
          seq_tv_train=seq_tv_train1(randperm(length(seq_tv_train1), minlength));
          
          seqTrain=[seq_d_train seq_r_train seq_el_train seq_tv_train];
          
          seq_d_test1=seq_d(find(indD==cvf));
          seq_r_test1=seq_r(find(indR==cvf));
          seq_el_test1=seq_el(find(indEL==cvf));
          seq_tv_test1=seq_tv(find(indTV==cvf));
          

          
          seqTest=[seq_d_test1 seq_r_test1 seq_el_test1 seq_tv_test1];
          Y_test=[ones(1,length(seq_d_test1)) 2*ones(1, length(seq_r_test1)) 3*ones(1,length(seq_el_test1)) 4*ones(1,length(seq_tv_test1))];
